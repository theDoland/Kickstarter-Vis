...
const RadarChart = function RadarChart(parent_selector, data, options) {
	...
	const rScale = d3.scaleLog()
		.base(Math.E)
		.range([Math.exp(0), radius])
		.domain([Math.exp(0), maxValue]);
	...
	// g.on("click", function() { console.log("test click"); });
	...
	axis.append("text")
		...
		.on("click", function() { 
			...
			var x = d3.select(this).text().split(/\s+/).reverse()[0]
			//console.log(x);
			...
			if (run) {
				d3.selectAll(".legend").style("font-size", "11px");
				d3.select(this).style("font-size", "16px");
				Interaction_Selected_Data = Interaction_Selected_Data.filter(word => word.main_category == x)
